...
namespace ViewModel
{
    public class CategoryDB : BaseDB
    {
        ...
        public CategoryList SelectByItemID(int itemID)
        {
            command.Parameters.Clear();

            command.CommandText = "SELECT [Category].* FROM [Category] INNER JOIN ItemCategory ON Category.CategoryID = ItemCategory.CategoryID WHERE ItemCategory.ItemID = @id";

            command.Parameters.Add(new OleDbParameter("@id", itemID));
